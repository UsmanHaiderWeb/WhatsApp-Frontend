...
const UnfriendUser = () => {
  ...
  const unfriendUserFunc = async (data: {password: string}) => {
    if(token && token != ''){
      try {
        let response = await axios.post(`${API_URL}/user/unfriend?token=${token}&id=${param.id}`, data)
        console.log("UNFRIEND USER RESPONSE: ", response);
        if(response.data?.status == 200) {
          dispatch(getAllChatsActions.removeChat(response.data.chatID));
          dispatch(userKeFriendAction.removeFriend(response.data.otherUser));
        ...
        console.log("UNFRIEND THE USER ERROR: ", error.message);
    ...
    <div className='w-full flex justify-between items-center'>
      <AlertDialog>
        <AlertDialogTrigger className='w-full flex justify-start items-center px-2 gap-x-2 opacity-80 hover:opacity-100 text-[14px] py-1'>
          <Ban size={17} />Unfriend this User
        </AlertDialogTrigger>
        <AlertDialogContent className='gap-y-0'>
          <AlertDialogHeader>
              <AlertDialogTitle>Unfriend this user</AlertDialogTitle>
              <AlertDialogDescription>If you want to unfriend this user, you have to enter your password. We are just making it sure that it is you.</AlertDialogDescription>
          </AlertDialogHeader>
          <form onSubmit={handleSubmit(unfriendUserFunc)}>
...
export default memo(UnfriendUser)